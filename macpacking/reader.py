...
class DatasetReader(ABC):
    ...
    def online(self, normalize=False):
        ...
        # nomralize the dataset if the normalize paramater is set to true
        if normalize:
    ...
    def _load_data_from_disk(self) -> WeightSet:
        for filename in self.__files:
            with open("./_datasets/jburkardt/" + filename, 'r') as reader:
                ...
                '''
                gets all the data at each line and stops when
...
class Normalized_reading():
    ...
    def Normalize(self) -> NomralizedWeightSet:
        for i in range(len(self.weights)):
            ...
            elif self.weights[i][0] < self.capacity*1//2 and \
                    self.weights[i][0] >= self.capacity*2//5:
                self.weights[i].append("B")
            elif self.weights[i][0] < self.capacity*2//5 and\
                    self.weights[i][0] >= self.capacity//3:
